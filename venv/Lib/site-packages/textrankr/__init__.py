...
# 말로는 트위터 형태소 분석기를 사용한댔지만 꼬꼬마 분석기를 사용한 것을 알 수 있다.
# 꼬꼬마는 실제 국어 형태소에 맞게 자세한 정보를 제공하지만 처리속도가 굉장히 느리고 정확도가 꽤나 떨어진다. 기사문들은 생각보다 비문이 많아서 조심해야 한다.
# 현재 분석기에서는 어차피 명사형만 모아서 분석하기 때문에, 트위터로 변경해도 크게 차이가 없을 듯싶다.
_kkma = Kkma()
